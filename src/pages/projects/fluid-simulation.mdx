...
To visualize the flow, we need to derive the Cartesian velocity components $u(x,y), v(x,y)$ from the stream function $\psi(r, \theta)$ (we need that because Manim takes in Cartesian coordinates). Doing this by hand can be terrible; it involves replacing $r, \theta$ with $x^2 + y^2, \text{atan}(y/x)$ respectively, and then differentiating with respect to x and y! From experience, it is terrible. The good news is that we can do it using sympy!
<br></br>
...
Here, StreamFunction takes a sympy expression of $\psi$ in polar coordinates, which is then supplied in coord. A suitable instance is this:
...
That's it! I hope you'll play around with it and get some awesome fluid animations! In the meantime, here are some of mine:
