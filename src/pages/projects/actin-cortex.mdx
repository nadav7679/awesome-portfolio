...
Now that we have a function calculating the non-flux terms, we can continue to the full iteration and solving of the system.
Notice, for example, that under the comment "Calculate inner-points" below, we can actually see equation (2) explicitly:
...
And that's it! With some plotting code, we get animations like the one above, or the following, which corresponds to a periodic boundary condition:
